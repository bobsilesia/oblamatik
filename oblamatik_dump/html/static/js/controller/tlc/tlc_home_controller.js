tlcApp.controller('TLCHomeController', function ($scope, $rootScope, tlcService, $stateParams, $translate, $location) {
    ...
    tlcService.getTLC(currentId).then(function (TLC) {
        ...
        $scope.testTemp = $scope.currentTemp;
    ...
    var tempPropellerOptions = {
        inertia: 0,
        onRotate: function () {
            //console.log("Temp:" +(20+(this.angle/10)));
            ...
            if ($scope.selectionActive == true) {
                if (clockWise == true) {
                    if ($scope.currentTemp > 40 && $scope.currentTemp < 60) {
                        $scope.currentTemp += 0.08;
                    }
                    else {
                        $scope.currentTemp += 0.15;
                    }
                }
                else {
                    if ($scope.currentTemp > 40 && $scope.currentTemp < 60) {
                        $scope.currentTemp -= 0.05;
                    }
                    else {
                        $scope.currentTemp -= 0.15;
                    }
                }
                if ($scope.currentTemp < tempStart || $scope.currentTemp > tempEnd) {
                    if ($scope.currentTemp < tempStart) {
                        $scope.currentTemp = tempStart;
                    }
                    else {
                        $scope.currentTemp = tempEnd;
                    }
                }
                $scope.$apply(function () {
