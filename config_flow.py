...
class OblamatikConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def _test_connection(self, host: str, port: int) -> bool:
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(url, timeout=5) as response:
                    if response.status == 200:
                        # Check content-type and handle both JSON and HTML
                        content_type = response.headers.get('content-type', '').lower()
                        if 'application/json' in content_type:
                            return True
                        elif 'text/html' in content_type:
                            # Try to parse HTML as JSON (some devices return JSON with HTML content-type)
                            try:
            ...
            _LOGGER.error(f"Connection test failed for {host}:{port}: {e}")
            return False
